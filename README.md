...
### 架构与配置
- **核心流程**：`server` 监听混合端口 -> `session` 读取首包 -> `router` 按检测器(`http`/`tls`/`ssh`/`prefix`/`always`)选择后端 -> 可选注入 PROXY v2 头/HTTP 真实源 IP 头 -> 雙向透传。
